...
from resources.lexical_analyzer.ply.lex import LexToken
...
class Grammar:
  ...
  def readInputLEX(self, data_file, output_file=sys.stdout, verbose=False):
    """
    Lê e valida um arquivo de entrada, na linguagem CC-2022-2, com auxílio do 
    analizador léxico LEX, utilizando a tabela de análise preditivo LL(1).
    
    Parameters
    ----------
    data_file: str
      caminho do arquivo de entrada
    output_file: str (default = sys.stdout)
      se especificado, imprime a pilha e a cadeia de entradas no caminho dado
    verbose: boolean (default = False)
      se verdadeiro imprime a pilha e a cadeia de entradas, em cada iteração
    """

    lex = LexicalAnalyzer()
    lex.build()
    lex.read_input(data_file.read())

    end = LexToken()
    end.value = end.type = '$'
    end.lineno = lex.lexer.lineno
    end.lexpos = lex.lexer.lexpos
    end.lexer = lex

    tokens = lex.token_list + [end]
    ...
    stack = ['$', grammar.nterminals[0]]
    read = tokens.pop(0)
    error_stack = [read.type]

    while True:
      if verbose:
        print(f'{stack} <- {read.type}: {read.value}', file=output_file)

      if read.type == stack[-1] and read.type == '$':
        return True

      elif read.type == stack[-1] and read.type != '$':
        stack.pop()
        read = tokens.pop(0)
        error_stack.append(read.type)

      elif stack[-1] in grammar.nterminals and grammar.llTable.get((stack[-1], read.type), ''):
        aux = grammar.llTable[(stack.pop(), read.type)][::-1]
        if aux != ['&']:
          stack += aux
          
      else:
        if read == end:
          error_message = "Fim inesperado do arquivo!\n"
        else:
          error_message  = f'Entrada inesperada "{read.value}", na linha {read.lineno} e coluna {lex.find_column(read)}!\n'
        
        error_message += f'Forma sentencial: {error_stack[-5:]}\n'
        error_message += f'Não-terminal/Literal esperado: "{stack[-1]}"\n'
        error_message += f'Entrada recebida: "{read.type}"'
